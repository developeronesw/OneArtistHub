...
async function paypalConnectCall(env,path,payload={},method='POST'){
  if(!env.ONEARTIST_CONNECT_URL||!env.ONEARTIST_CONNECT_TOKEN)throw new Error('PayPal Connect service is not configured.');
  const base=String(env.ONEARTIST_CONNECT_URL).replace(/\/+$/,'');
  const headers={authorization:`Bearer ${env.ONEARTIST_CONNECT_TOKEN}`,'content-type':'application/json'};
...
async function paypalWorkerConfigured(env){return !!(env.ONEARTIST_CONNECT_URL&&env.ONEARTIST_CONNECT_TOKEN);}
