...
if(!/installationAuthorized/.test(connectWorker)||!/CONNECT_INSTALLATION_TOKENS/.test(connectWorker)||!/url\.pathname==='\/onboard\/start'/.test(connectWorker)||!/url\.pathname==='\/onboard\/status'/.test(connectWorker)||!/if\(!authorized\(req,env\)\)/.test(connectWorker)){console.error('FAIL pre-merchant installation onboarding authorization');fail=true}else console.log('PASS pre-merchant installation onboarding authorization');
