...
function installationId(){return `OAH_SITE_${Array.from(crypto.getRandomValues(new Uint8Array(16)),b=>b.toString(16).padStart(2,'0')).join('')}`;}
function bootstrapToken(){return `OAH_BOOT_${Array.from(crypto.getRandomValues(new Uint8Array(32)),b=>b.toString(16).padStart(2,'0')).join('')}`;}
async function connectTokenHash(value){return bytesToB64(new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(String(value))))).replace(/=+$/,'').replace(/\+/g,'-').replace(/\//g,'_')}
async function storedConnectCredential(env){try{const row=await env.DB.prepare(`SELECT data_enc FROM integrations WHERE provider='oneartist_connect'`).first();return row?await decrypt(env,row.data_enc):null}catch{return null}}
async function saveConnectCredential(env,value){const encrypted=await encrypt(env,value);await env.DB.prepare(`INSERT INTO integrations(provider,data_enc,updated_at) VALUES(?,?,?) ON CONFLICT(provider) DO UPDATE SET data_enc=excluded.data_enc,updated_at=excluded.updated_at`).bind('oneartist_connect',encrypted,now()).run()}
async function storedConnectProvisioning(env){try{const row=await env.DB.prepare(`SELECT data_enc FROM integrations WHERE provider='oneartist_connect_pending'`).first();return row?await decrypt(env,row.data_enc):null}catch{return null}}
async function saveConnectProvisioning(env,value){const encrypted=await encrypt(env,value);await env.DB.prepare(`INSERT INTO integrations(provider,data_enc,updated_at) VALUES(?,?,?) ON CONFLICT(provider) DO UPDATE SET data_enc=excluded.data_enc,updated_at=excluded.updated_at`).bind('oneartist_connect_pending',encrypted,now()).run()}
async function clearConnectProvisioning(env){await env.DB.prepare(`DELETE FROM integrations WHERE provider='oneartist_connect_pending'`).run()}
async function connectCredential(env){const stored=await storedConnectCredential(env);if(stored?.token&&stored?.installationId)return stored;if(env.ONEARTIST_CONNECT_TOKEN)return {token:String(env.ONEARTIST_CONNECT_TOKEN),installationId:''};return null}
async function registerConnectInstallation(env,origin){if(!env.ONEARTIST_CONNECT_URL||!env.ONEARTIST_CONNECT_PROVISIONING_SECRET||!/^https:\/\//i.test(String(origin||'')))return {configured:false,state:'not_configured'};const base=String(env.ONEARTIST_CONNECT_URL).replace(/\/+$/,''),route=String(origin).replace(/\/+$/,''),pending=await storedConnectProvisioning(env)||{installationId:installationId(),bootstrapToken:bootstrapToken(),createdAt:now()};await saveConnectProvisioning(env,pending);const activate=await fetch(`${base}/installations/activate`,{method:'POST',headers:{authorization:`Bearer ${env.ONEARTIST_CONNECT_PROVISIONING_SECRET}`,'content-type':'application/json'},body:JSON.stringify({installationId:pending.installationId,url:route,bootstrapHash:await connectTokenHash(pending.bootstrapToken)})});if(!activate.ok)throw new Error('Connect activation could not be created.');const r=await fetch(`${base}/installations/register`,{method:'POST',headers:{authorization:`Bearer ${pending.bootstrapToken}`,'content-type':'application/json'},body:JSON.stringify({installationId:pending.installationId,url:route})});let result={};try{result=await r.json()}catch{}if(!r.ok||!result.ok||!result.token)throw new Error('Connect registration could not be completed.');await saveConnectCredential(env,{installationId:pending.installationId,token:String(result.token),registeredAt:now()});await clearConnectProvisioning(env);return {configured:true,registered:true,state:'registered'};}
async function ensureConnectInstallation(env,origin){if(await connectCredential(env))return {configured:true,registered:false,state:'already_registered'};return registerConnectInstallation(env,origin)}
...
async function paypalConnectState(env){ const s=await getSettings(env); return s.paypalConnect||{}; }
async function paypalConnectCall(env,path,payload={},method='POST'){
  const credential=await connectCredential(env);if(!env.ONEARTIST_CONNECT_URL||!credential?.token)throw new Error('PayPal Connect service is not configured.');
  ...
  const r=await fetch(`${base}${path}`,{method,headers,body:method==='GET'?undefined:JSON.stringify(payload)});
...
async function paypalOrderDetails(env,paypalOrderId){
  const state=await paypalConnectState(env);
  if(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)){
    const j=await paypalConnectCall(env,'/paypal/order',{merchantId:state.merchantId,orderId:paypalOrderId});
    return {ok:true,order:j.order||j,status:200};
  }
  return {ok:false,order:{},status:503};
}
async function paypalCaptureOrRecover(env,paypalOrderId,requestId){
  const state=await paypalConnectState(env);
  if(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)){
    try{
      const j=await paypalConnectCall(env,'/paypal/capture',{merchantId:state.merchantId,orderId:paypalOrderId,requestId:cleanText(requestId,38)});
      return {ok:true,order:j.order||j,recovered:false,status:200};
    }catch(err){
      const details=await paypalOrderDetails(env,paypalOrderId);
      ...
      return {ok:false,order:details.order||{},recovered:false,status:502,error:cleanText(err?.message||err,500)};
    }
  }
  return {ok:false,order:{},recovered:false,status:503,error:'PayPal Connect is not configured.'};
}
...
export async function route(req,env,url,ctx){
  ...
  const installed=await isInstalled(env); if(installed){await ensureUpgrade012(env);await ensureUpgrade013(env);await ensureUpgrade020(env);await ensureUpgrade022(env);await ensureUpgrade023(env);try{await ensureConnectInstallation(env,url.origin)}catch{}if(ctx?.waitUntil)ctx.waitUntil(processEmailQueue(env,3));}
  ...
  if(p==='setup' && method==='POST'){
    ...
    let connectRegistration={configured:false,state:'not_configured'};try{connectRegistration=await ensureConnectInstallation(env,url.origin)}catch{connectRegistration={configured:true,registered:false,state:'failed'}};
    return json({ok:true,connectRegistration:{configured:!!connectRegistration.configured,registered:!!connectRegistration.registered,state:connectRegistration.state}});
  }
  ...
  if(p==='paypal/config' && method==='GET'){
    const settings=await getSettings(env), state=settings.paypalConnect||{};
    if(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)){
      try{const c=await paypalConnectCall(env,'/config',{},'GET');return json({ok:true,configured:true,connected:true,merchantId:state.merchantId,clientId:c.clientId||'',environment:c.environment||'sandbox',webhookConfigured:!!c.webhookConfigured,currency:settings.commerce?.currency||'USD'});}
      catch(e){return json({ok:false,error:cleanText(e.message||e,500),configured:false},503);}
    }
    return json({ok:true,configured:false,connected:false,merchantId:'',clientId:'',environment:'sandbox',webhookConfigured:false,currency:settings.commerce?.currency||'USD'});
  }
  if(p==='paypal/create-order' && method==='POST'){
    ...
    const settings=await getSettings(env), commerce=settings.commerce||{}, currency=currencyCode(commerce.currency), shipping=cart.hasPhysical?money(commerce.flatShipping):0, total=money(cart.subtotal+shipping), state=settings.paypalConnect||{};
    ...
    let j;
    if(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)){
      try{j=await paypalConnectCall(env,'/paypal/create-order',{merchantId:state.merchantId,payload,requestId:id()});j=j.order||j;}catch(e){return json({ok:false,error:cleanText(e.message||'PayPal order creation failed.',500)},502);}
    }else return json({ok:false,error:'Connect PayPal in Settings before accepting payments.'},503);
    if(!j?.id)return json({ok:false,error:'PayPal order creation failed.'},502);
  ...
  if(p==='paypal/capture' && method==='POST'){
    const state=await paypalConnectState(env); if(!(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)))return json({ok:false,error:'Connect PayPal in Settings before accepting payments.'},503); const b=await body(req), paypalOrderId=cleanText(b.paypalOrderId,80); if(!paypalOrderId)return json({ok:false,error:'Missing PayPal order.'},400);
    ...
    const requestId=('OAHCAP-'+paypalOrderId).slice(0,38), cap=await paypalCaptureOrRecover(env,paypalOrderId,requestId), j=cap.order;
  ...
  if(p==='paypal/webhook' && method==='POST')return json({ok:false,error:'PayPal webhooks are received by the central Connect Worker.'},410);
  if(p==='paypal/webhook/internal' && method==='POST'){
    const expectedToken=String(env.ONEARTIST_CONNECT_TOKEN||''),receivedToken=req.headers.get('x-oneartist-connect-token')||'';
    const connectedMerchant=String((await paypalConnectState(env)).merchantId||'');
    if(!expectedToken||receivedToken!==expectedToken||!connectedMerchant||req.headers.get('x-oneartist-connect-merchant')!==connectedMerchant)return json({ok:false,error:'Unauthorized webhook delivery.'},401);
    const raw=await req.text(); let event={}; try{event=JSON.parse(raw)}catch{return json({ok:false,error:'Invalid webhook body.'},400)}
    const eventId=cleanText(event.id,160), eventType=cleanText(event.event_type,120); if(!eventId)return json({ok:false,error:'Webhook event ID is missing.'},400);
    ...
    try{
      ...
      if(eventType==='CHECKOUT.ORDER.APPROVED'){
        const existing=await env.DB.prepare('SELECT id FROM orders WHERE paypal_order_id=?').bind(paypalOrderId).first(); if(!existing){const checkout=await env.DB.prepare('SELECT * FROM checkout_sessions WHERE paypal_order_id=?').bind(paypalOrderId).first(); if(checkout&&new Date(checkout.expires_at)>new Date()){const items=parseJson(checkout.cart_json,[]),cap=await paypalCaptureOrRecover(env,paypalOrderId,('OAWH-'+paypalOrderId).slice(0,38)),jj=cap.order;if(cap.ok&&jj.status==='COMPLETED'&&paypalCaptureStatus(jj)==='COMPLETED')await finalizeCapturedOrder(env,url,paypalOrderId,jj,items,checkout,ctx,{})}}
        ...
        else {const checkout=await env.DB.prepare('SELECT * FROM checkout_sessions WHERE paypal_order_id=?').bind(paypalOrderId).first();if(checkout&&new Date(checkout.expires_at)>new Date()){const items=parseJson(checkout.cart_json,[]),od=await paypalOrderDetails(env,paypalOrderId),oj=od.order;if(od.ok&&oj.status==='COMPLETED')await finalizeCapturedOrder(env,url,paypalOrderId,oj,items,checkout,ctx,{})}}
  ...
  if(p==='admin/paypal/test'&&method==='POST'){if(!requireCsrf(req,user))return json({ok:false,error:'CSRF validation failed.'},403);const state=await paypalConnectState(env);if(state.status!=='connected'||!await paypalWorkerConfigured(env))return json({ok:false,error:'Connect PayPal first.'},400);const c=await paypalConnectCall(env,'/config',{},'GET');return json({ok:true,environment:c.environment||'sandbox',authenticated:true,connected:true});}
  ...
  if(p==='admin/paypal/config'&&method==='GET'){ const state=await paypalConnectState(env); if(state.status==='connected'&&await paypalWorkerConfigured(env)){let c={};try{c=await paypalConnectCall(env,'/config',{},'GET')}catch{} return json({ok:true,configured:true,mode:'connect',clientId:c.clientId||'',environment:c.environment||'sandbox',webhookConfigured:false,merchantId:state.merchantId||'',status:state.status||'connected'});} return json({ok:true,configured:false,mode:'connect',clientId:'',environment:'sandbox',webhookConfigured:false,merchantId:'',status:state.status||'not_connected'}); }
  if(p==='admin/integrations'&&method==='POST'){ if(!requireCsrf(req,user))return json({ok:false,error:'CSRF validation failed.'},403); const b=await body(req), provider=cleanText(b.provider,40); if(!['dropbox','s3','email'].includes(provider))return json({ok:false,error:'PayPal is managed through Connect PayPal; configure only the central Worker.'},400); let data={}; if(provider==='dropbox'){let previous=null;try{const r=await env.DB.prepare(`SELECT data_enc FROM integrations WHERE provider='dropbox'`).first();if(r)previous=await decrypt(env,r.data_enc)}catch{} data={accessToken:String(b.accessToken||previous?.accessToken||'')};} else if(provider==='s3'){let previous=null;try{previous=await s3Config(env)}catch{} data={endpoint:cleanText(b.endpoint||previous?.endpoint||'',600).replace(/\/+$/,''),region:cleanText(b.region||previous?.region||'us-east-1',64),bucket:cleanText(b.bucket||previous?.bucket||'',128),accessKeyId:cleanText(b.accessKeyId||previous?.accessKeyId||'',256),secretAccessKey:String(b.secretAccessKey||previous?.secretAccessKey||''),forcePathStyle:b.forcePathStyle!==false};} else { let previous=null; try{previous=await emailConfig(env)}catch{} const service=['resend','brevo','cloudflare','smtp'].includes(b.service)?b.service:'resend'; data={service,apiKey:String(b.apiKey||previous?.apiKey||''),apiToken:String(b.apiToken||previous?.apiToken||''),accountId:cleanText(b.accountId||previous?.accountId||'',80),smtpHost:cleanText(b.smtpHost||previous?.smtpHost||'',220),smtpPort:Number(b.smtpPort||previous?.smtpPort||587),smtpSecure:!!b.smtpSecure,smtpUser:cleanText(b.smtpUser||previous?.smtpUser||'',220),smtpPassword:String(b.smtpPassword||previous?.smtpPassword||''),fromName:cleanText(b.fromName||previous?.fromName||'',120),fromEmail:cleanText(b.fromEmail||previous?.fromEmail||'',220).trim(),replyTo:cleanText(b.replyTo||previous?.replyTo||'',220).trim()}; } const emailMissing=provider==='email'&&(!data.fromEmail||(data.service==='cloudflare'?(!data.apiToken||!data.accountId):data.service==='smtp'?(!env.SMTP_SEND||!data.smtpHost||!data.smtpUser||!data.smtpPassword):!data.apiKey)); if((provider==='dropbox'&&!data.accessToken)||(provider==='s3'&&(!data.endpoint||!data.bucket||!data.accessKeyId||!data.secretAccessKey))||emailMissing)return json({ok:false,error:'Required secret or sender information is missing.'},400); const e=await encrypt(env,data); await env.DB.prepare(`INSERT INTO integrations(provider,data_enc,updated_at) VALUES(?,?,?) ON CONFLICT(provider) DO UPDATE SET data_enc=excluded.data_enc,updated_at=excluded.updated_at`).bind(provider,e,now()).run(); return json({ok:true}); }
  ...
  if(refundMatch&&method==='POST'){
    ...
    const state=await paypalConnectState(env); if(!(state.status==='connected'&&state.merchantId&&await paypalWorkerConfigured(env)))return json({ok:false,error:'Connect PayPal in Settings before issuing refunds.'},503);
    ...
    const payload=requested===remaining?{}:{amount:{value:requested.toFixed(2),currency_code:o.currency}}; let j;
    const paypalState=await paypalConnectState(env);
    try{j=await paypalConnectCall(env,'/paypal/refund',{merchantId:paypalState.merchantId,captureId,requestId:('OAREF-'+id()).slice(0,38),payload});j=j.refund||j;}catch(e){return json({ok:false,error:cleanText(e.message||'PayPal refund failed.',500)},502);}
    if(!j||!['COMPLETED','PENDING'].includes(j.status))return json({ok:false,error:cleanText(j.message||'PayPal refund failed.',500)},502);
    if(j.status==='PENDING'){
